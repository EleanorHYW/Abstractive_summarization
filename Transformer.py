...
class ElmoEmbedder(nn.Module):
    def __init__(self, requires_grad=False, dropout=0.5):
        ...
        self.elmo = Elmo(config.options_file,config.weight_file,num_output_representations=2,requires_grad=requires_grad,dropout=dropout)
...
class BertEmbedder(nn.Module):
    def __init__(self, pretrained_model: str, requires_grad: bool = False, top_layer_only: bool = False):
        ...
        # TODO,
        # set num_output_representations=3 may result to better performance
        self.bert = PretrainedBertEmbedder(pretrained_model="bert-large-uncase")
    ...
    def forward(self, stncs, tgt_flag=False, idx=-1):
        ...
        :return mask: 0-1 tensor in shape [batch, max_len], 0 for padding
        """
        bert_ids = self.batch_to_ids(stncs, tgt_flag).cuda()
        bert_output = self.bert(bert_ids)
        embeddings = bert_output['elmo_representations'][idx]
        mask = bert_output['mask']
        if tgt_flag:
            b_size = len(stncs)
            embeddings = [embeddings[i][i//b_size].unsqueeze(0)
                          for i in range(embeddings.shape[0])]
            embeddings = torch.cat(embeddings, dim=0).view(bert_ids.shape[1], b_size, -1)
            embeddings = embeddings.transpose(0, 1)
            mask = mask[-b_size:]
        return embeddings, mask
